-- ============================================================
-- 039 — harden upsert_refund_with_inventory batch restock (CodeRabbit B3-B4)
-- ============================================================
-- Two new BLOCK items from the second CodeRabbit pass on 037:
--
-- B3 (new): Wholesale batch adjustment validation checked each
--   p_batch_adjustments row independently. Duplicate rows for the same
--   batch_id could each pass the drawn-qty cap while their combined total
--   exceeded it. Fix: aggregate by batch_id, validate the summed quantity.
--
-- B4 (new): When restock=true but p_batch_adjustments is absent/empty, the
--   RPC fell through to the retail inventory path even for wholesale sales
--   (those with sale_item_batches depletions). Fix: detect wholesale server-
--   side from sale_item_batches; if wholesale, require non-empty adjustments.
...
begin
  ...
  if v_is_wholesale then
    ...
    -- B3: Aggregate p_batch_adjustments by batch_id before validating. Duplicate
    -- rows for the same batch now sum their quantities before the drawn-qty cap,
    -- so two rows of qty=3 against a drawn=5 are correctly rejected together.
