...
class SeedService {
  ...
  partitionDelta(List<Map<String, dynamic>> rows, {required bool collectDead}) {
  ...
  Future<void> _seedBranches(String shopId) => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertBranches(
      rows
          .map(
            (b) => LocalBranchesCompanion(
  ...
  Future<void> _seedSettings(String shopId) => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertSettings(
      rows
          .map(
            (s) => LocalShopSettingsCompanion(
              shopId: Value(shopId),
  ...
  Future<void> _seedPaymentMethods(String shopId) => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertPaymentMethods(
      rows
          .map(
            (m) => LocalPaymentMethodsCompanion(
  ...
  Future<void> _seedCategories(String shopId) => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertCategories(
      rows
          .map(
            (c) => LocalProductCategoriesCompanion(
  ...
  Future<void> _seedUnits() => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertUnits(
      rows
          .map(
            (u) => LocalMeasurementUnitsCompanion(
  ...
  Future<void> _seedProductBatches() => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertProductBatches(
      rows
          .map(
            (b) => LocalProductBatchesCompanion(
              id: Value(b['id'] as String),
              ...
              receivedAt: Value(
                DateTime.tryParse(b['received_at']?.toString() ?? '') ?? _now,
              ),
  ...
  Future<void> _seedSaleItemBatches() => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertSaleItemBatches(
      rows
          .map(
            (s) => LocalSaleItemBatchesCompanion(
  ...
  Future<void> _seedBatchAdjustments() => _deltaPull(
    ...
    fetch: (cursorIso) async {
      var q = _client
          ...
          .select(
            'id, batch_id, branch_id, product_id, quantity_delta, reason, created_by, created_at, updated_at, deleted_at, refund_id, sale_item_id',
          );
      if (cursorIso != null) q = q.gte('updated_at', cursorIso);
      return (await q as List).cast<Map<String, dynamic>>();
    },
    applyLive: (rows) => _db.upsertBatchAdjustments(
      rows
          .map(
            (a) => LocalBatchAdjustmentsCompanion(
              ...
              createdAt: Value(
                DateTime.tryParse(a['created_at']?.toString() ?? '') ?? _now,
              ),
              syncedAt: Value(_now),
              isSynced: const Value(true),
              refundId: Value(a['refund_id'] as String?),
              saleItemId: Value(a['sale_item_id'] as String?),
            ),
          )
          .toList(),
    ),
  ...
  Future<void> _seedRefunds() => _deltaPull(
    ...
    applyLive: (rows) async {
      final pending = (await _db.getPendingRefunds()).map((r) => r.id).toSet();
      await _db.upsertRefunds(
        rows
            .where((r) => !pending.contains(r['id'] as String))
            .map(
              (r) => LocalRefundsCompanion(
                ...
                createdAt: Value(
                  DateTime.tryParse(r['created_at']?.toString() ?? '') ?? _now,
                ),
                syncedAt: Value(_now),
                isSynced: const Value(true),
              ),
  ...
  Future<void> _seedRefundItems() => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertRefundItems(
      rows
          .map(
            (i) => LocalRefundItemsCompanion(
  ...
  Future<void> _seedProfiles(String shopId) => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertProfiles(
      rows
          .map(
            (p) => LocalProfilesCompanion(
  ...
  Future<void> _seedProducts(String shopId) => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertProducts(
      rows
          .map(
            (p) => LocalProductsCompanion(
              id: Value(p['id'] as String),
              ...
              measurementUnitAbbr: Value(
                (p['measurement_units']
                            as Map<String, dynamic>?)?['abbreviation']
                        as String? ??
                    '',
              ),
              lowStockThreshold: Value(_dec(p['low_stock_threshold'])),
              sellingPrice: Value(
                p['selling_price'] != null ? _dec(p['selling_price']) : null,
              ),
              costPrice: Value(
  ...
  Future<void> _seedStock(String branchId) => _deltaPull(
    ...
    fetch: (cursorIso) async {
      var q = _client
          .from('inventory')
          .select('product_id, quantity, expiry_date, updated_at, deleted_at')
  ...
  Future<void> _seedExpenseCategories(String shopId) => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertExpenseCategories(
      rows
          .map(
            (c) => LocalExpenseCategoriesCompanion(
              id: Value(c['id'] as String),
              shopId: Value(c['shop_id'] as String?),
              name: Value(c['name'] as String),
              syncedAt: Value(_now),
  ...
  LocalSalesCompanion _saleCompanion(Map<String, dynamic> r) =>
      LocalSalesCompanion(
        ...
        customerName: Value(
          (r['customers'] as Map<String, dynamic>?)?['name'] as String?,
        ),
  ...
  Future<void> _seedExpenses(String shopId) => _deltaPull(
    ...
    applyLive: (rows) async {
      final pendingIds = (await _db.getPendingExpenses())
          .map((e) => e.id)
          .toSet();
      for (final e in rows) {
        final id = e['id'] as String;
        if (pendingIds.contains(id)) continue;
        await _db.insertOrReplaceExpense(
  ...
  Future<void> _seedCreditPayments() => _deltaPull(
    ...
    applyLive: (rows) => _db.upsertCreditPayments(
      rows
          .map(
            (p) => LocalCreditPaymentsCompanion(
              id: Value(p['id'] as String),
